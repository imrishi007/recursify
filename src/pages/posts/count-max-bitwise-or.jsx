...
public:
  ...
  const pythonCode = `class Solution:
    def countMaxOrSubsets(self, nums: List[int]) -> int:
        ...
        n = len(nums)
        # Memoization table: key is (index, current_or)
        memo = {}
        ...
        def calc(ind, curr):
            ...
            # 1. Take: Include nums[ind]
            take = calc(ind + 1, curr | nums[ind])
            
            # 2. Not take: Exclude nums[ind]
            not_take = calc(ind + 1, curr)
            
            # Store result
            memo[state] = take + not_take
  ...
  const javaCode = `class Solution {
    ...
    public int countMaxOrSubsets(int[] nums) {
        ...
        // Initialize DP table
        // Dimensions: nums.length for indices, maxOr + 1 for OR values
        memo = new Integer[nums.length][maxOr + 1];
        
        return calc(nums, 0, 0);
    }

    private int calc(int[] nums, int ind, int curr) {
        // Base case: If we've processed all elements
        if (ind == nums.length) {
        ...
        // Memoization check
        if (memo[ind][curr] != null) {
            return memo[ind][curr];
        }

        // 1. Take: Include nums[ind]
        int take = calc(nums, ind + 1, curr | nums[ind]);
        
        // 2. Not take: Exclude nums[ind]
        int notTake = calc(nums, ind + 1, curr);

        // Store result
        return memo[ind][curr] = take + notTake;
    }
}`;
    ...
    <div className="post-content">
      ...
      <section>
        ...
        <div className="content-box">
          ...
          <ul>
            <li>
              <strong>Pure Recursion (Without Memoization):</strong>
              <ul>
                <li>
                  <strong>Time Complexity: O(2^N)</strong>
                  <p>
                    This is what we call <code>exponential time</code>, which sounds scary, and it often is! Why 2^N? Think of it like this: for every single one of the <code>N</code> elements in your <code>nums</code> array, our recursive function faces two fundamental choices – <code>TAKE</code> it or <code>NOT TAKE</code> it. Each choice branches out further. This leads to an explosion of possibilities, with our function re-calculating the same "sub-problems" over and over again.
                  </p>
                </li>
                <li>
                  <strong>Space Complexity: O(N)</strong>
                  <p>
                    This refers to the memory used by the <code>recursion call stack</code>. Every time our <code>calc</code> function calls itself, it adds a new "frame" to the stack. In the worst-case scenario, the recursion can go as deep as <code>N</code> levels.
                  </p>
                </li>
              </ul>
            </li>
            <li>
              <strong>Memoized Recursion (Dynamic Programming `dp` solution):</strong>
              <ul>
                <li>
                  <strong>Time Complexity: O(N * maxOr)</strong>
                  <p>
                    Now this is where the magic happens! With memoization, we introduce our <code>dp</code> table. This table ensures that each unique state – defined by <code>(current_index, current_OR_value)</code> – is calculated <code>only once</code>. So, the total time boils down to the number of unique states we can possibly visit, which is roughly <code>N</code> (for the index) multiplied by <code>maxOr</code> (for the possible OR values).
                  </p>
                </li>
                <li>
                  <strong>Space Complexity: O(N * maxOr)</strong>
                  <p>
                    And here's the trade-off! To store all those calculated results so we don't repeat work, we need our <code>dp</code> table. Its size is determined by the number of indices (<code>N</code>) and the range of possible <code>current_OR_value</code>s (up to <code>maxOr</code>). Trading a bit more memory for a massive speed boost is a pretty sweet deal!
                  </p>
                </li>
              </ul>
            </li>
